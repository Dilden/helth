...
>
	...
	<span class="col-start-1 col-end-2 md:col-start-2 md:col-end-5">
		...
		<input
			class="mx-auto my-2 block w-11/12 bg-slate-100"
			type="text"
			id="recipeName"
			name="name"
			required
		...
		{#if reactiveItems?.length}
			...
			>
				{#each reactiveItems as item}
					...
					<div
						class="flex w-full flex-row items-center justify-between gap-y-1 odd:bg-(--back-color) lg:w-auto lg:flex-col lg:justify-start {item.name
							.toLowerCase()
